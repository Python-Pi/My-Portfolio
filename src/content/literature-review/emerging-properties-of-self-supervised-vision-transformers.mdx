...
$$\qquad$$ Transformers have emerged as a powerful architecture in computer vision but in the early days of its adoption mostly with supervised learning,
...
One of the main reasons transformers dominated the Natural Language Processing (NLP) field was due to the use of self-supervised methods,
...
$$\qquad$$ A large amount of work on self-supervised learning focuses on **discriminative approaches** that learn representations by comparing different views of the same image.
...
are treated as negatives. The use of strong augmentations is crucial here, as it defines the invariance the model should learn and prevents trivial solutions.
...
$$\qquad$$ Self learning is a strategy where the model tries the propagate information from a set of labelled data
to a larger set of unlabelled data. The model is first trained on the labelled data and then used
to predict the labels for the unlabelled data (also called pseudo-labelling). The model is then retrained on the combined dataset of labelled and pseudo-labelled data.
...
$$\qquad$$ The idea of knowledge distillation can be pushed even further by removing the need for any labels at all. In this case, 
...
[See Blog: What is Collapse in Self-Supervised Learning?](/blogs/what-is-collapse-in-self-supervised-learning)
...
Here are some of the impressive results achieved by DINO with ViTs on various datasets without any fine tuning:
<img
...
- Self-supervised ViT features perform particularly well with a basic nearest neighbors classifier (k-NN) without any fine tuning, 
...
**Softmax Function**: A mathematical function that converts a vectors of numbers into probabilities that sum to 1.
...
$$\rightarrow$$ [Why Transformers Don't Use Batch Normalization](/blogs/why-transformers-dont-use-batch-normalization)

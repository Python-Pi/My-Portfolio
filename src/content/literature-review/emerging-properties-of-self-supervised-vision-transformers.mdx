...
Given a fixed teacher network $$ g_t $$, the student network $$ g_s $$ is trained to minimize the cross-entropy loss between the two distributions:
...
In order to train both the student and teacher networks without any labels, we construct different distoretd views 
